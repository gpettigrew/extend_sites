...
function updateCalendar() {
    ...
    firstSunday.setUTCDate(firstDayOfMonth.getUTCDate() - firstDayOfMonth.getUTCDay());  // Adjust to the first Sunday

    for (let i = 0; i < 42; i++) {  // Always display 6 weeks
        const currentDay = new Date(firstSunday);
        currentDay.setUTCDate(firstSunday.getUTCDate() + i);

        const dayCell = document.createElement('div');
        dayCell.className = 'day';
        dayCell.textContent = currentDay.getUTCDate();
        dayCell.dataset.date = new Date(currentDay.getTime() + (timeZoneOffset * 60 * 60 * 1000)).toISOString().slice(0, 10);
        dayCell.onclick = () => selectDate(new Date(currentDay.getTime() + (timeZoneOffset * 60 * 60 * 1000)));

        console.log(`Creating cell for date: ${currentDay.toISOString().slice(0, 10)}`);  // Log the date being created

        if (currentDay.getUTCMonth() !== month) {
...
function highlightSelectedDate() {
    ...
    days.forEach(day => {
        const dayDate = new Date(day.dataset.date).setUTCHours(0, 0, 0, 0);
        const selectedDateTime = new Date(selectedDate).setUTCHours(0, 0, 0, 0);
